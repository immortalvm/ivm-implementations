...
type AsRunDelegate = delegate of string * bool * string
                                 * FileInfo * DirectoryInfo * DirectoryInfo
                                 * string option * bool * DirectoryInfo * seq<FileSystemInfo>
                                 * seq<FileSystemInfo> -> int
type CheckDelegate = delegate of string * bool * string
                                 * string option * bool * DirectoryInfo * seq<FileSystemInfo>
...
let main argv =
    ...
    let loc = argOpt "--loc" "Specify memory location" <| Argument<string>("loc") |> alias "-L"
    ...
    let rootCommand =
        extend (RootCommand("ivm", Description="iVM Assembler and VM")) [
            ...
            com "run" "Execute binary" [
                mem; trace; loc; arg; inp; out
                (fileArg "binary file" "Name of binary file").ExistingOnly()
            ] <| CommandHandler.Create(fun memory trace loc arg inp out ``binary file`` ->
                    run (oMem memory) (fName ``binary file``) (oName arg) (oName inp) (oName out) trace (oMem loc))

            com "as-run" "Assemble and run" [
                mem; trace; loc; arg; inp; out
                entry; noopt; root; sources; libraries
            ] <| CommandHandler.Create(new AsRunDelegate(fun memory trace loc arg inp out entry noopt root ``source files`` library ->
                         asRun (fNames ``source files``) (oName root) (fNames library) entry (oMem memory) (oName arg) (oName inp) (oName out) trace (oMem loc) noopt))

            com "check" "Assemble, run and check final stack" [
                mem; trace; loc
                entry; noopt; root; sources; libraries
            ] <| CommandHandler.Create(new CheckDelegate(fun memory trace loc entry noopt root ``source files`` library ->
                    doCheck (fNames ``source files``) (oName root) (fNames library) entry (oMem memory) trace  (oMem loc) noopt |> printfn "%s"))

...
let private src (filename: string) =
    let str () : Stream = upcast File.OpenRead filename
    Path.GetFileNameWithoutExtension filename, str

let assem source binary symbols =
    let ao = doAssemble [src source] [] // TODO

...
(defun test-9-all ()
  (let* ((m (make-machine 9 :groups '(core bit-1 bit-2 math-1 math-2 logic io))))
    (asm m `((nop)
             (push1 CONTINUE_2)
             (push1 #x00)
             (push1 #x01)
             ...
             (data #xAE)))
    (let ((m-orig (machine-copy m)))
      (print-machine m :end (machine-index m) :format :plain)
      (run m)
      (print-machine m :start #x158 :format :plain)
      #+nil
      (print-machine-diff m m-orig :format :plain))))

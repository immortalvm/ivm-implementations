...
(defconstant +jump_zero+  #x03)
(defconstant +set_sp+     #x04)
(defconstant +get_pc+     #x05)
(defconstant +get_sp+     #x06)
(defconstant +push0+      #x07)
(defconstant +push1+      #x08)
...
(define-opcode +jump_zero+ (m (pc) :group core)
  (let ((a (sigext 1 (fetch m 1))))
    (when (zerop (pop* m))
      (incf pc a))))
          ...
          (etypecase entry
            ...
            (cons (destructuring-bind (opcode . args) entry
                    (ecase opcode
                      ...
                      (jump_zero (newloc machine 1 +jump_zero+)

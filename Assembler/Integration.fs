...
let doRun memorySize binary arg inputDir outputDir traceSyms loc =
    try
        // Default memory size: 16 MiB (for now)
        execute (valueOr (1UL <<< 24) memorySize) binary arg inputDir outputDir traceSyms loc |> Seq.map int64
    with
